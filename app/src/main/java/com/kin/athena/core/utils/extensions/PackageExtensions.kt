...
fun Application.getApplicationName(packageManager: PackageManager): String? =
  try {
    ...
    appInfo?.let { packageManager.getApplicationLabel(appInfo).toString() }
  } catch (e: NameNotFoundException) {
    null
  }
  ...
  return try {
    ...
    when {
      Build.VERSION.SDK_INT >= Build.VERSION_CODES.TIRAMISU &&
        ...
        useDynamicIcon -> {
        icon.monochrome?.let { monochrome -> applyTint(monochrome, tintColor).processIcon(context) }
          ?: icon.foreground?.processIcon(context)
          ?: icon
      }

      else -> {
        icon.processIcon(context)
      }
    }
  } catch (e: NameNotFoundException) {
  ...
  try {
    ...
    permissions.mapNotNull { permission ->
      try {
        ...
        val description =
          permissionInfo.loadLabel(packageManager).toString().replaceFirstChar { it.uppercase() }
