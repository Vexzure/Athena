...
fun NavController.safeNavigate(route: String) {
  ...
  if (currentDestinationRoute != null) {
    ...
    if (
      currentDestinationRoute != targetFirstSegment ||
      (segmented.size > 1 && segmented[1] == "lock")
    ) {
      navigate(route)
    }
  } else {
...
fun NavOptionsBuilder.popUpToTop(navController: NavController) {
  popUpTo(navController.currentBackStackEntry?.destination?.route ?: return) { inclusive = true }
}
