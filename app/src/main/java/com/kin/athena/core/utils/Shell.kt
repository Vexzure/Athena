...
import com.kin.athena.core.utils.Shell.Command
 ...
 * returning stdout, stderr, and the last exit code as a [Command.Result] when a command is
 * complete.
 *
 * Example usage:
 *
...
class Shell
  ...
  constructor(
     ...
     * @param environment varargs of all environment variables as a [Pair] which are included with the
     *   system environment.
     */
    constructor(
      shell: String,
      vararg environment: Pair<Variable, Value>,
     ...
     */
    fun addOnCommandResultListener(listener: OnCommandResultListener) =
      apply {
     ...
     * @return This shell instance for chaining calls.
     */
    fun removeOnCommandResultListener(listener: OnCommandResultListener) =
      apply {
     ...
     * @return This shell instance for chaining calls.
     */
    fun addOnStdoutLineListener(listener: OnLineListener) = apply { onStdOutListeners.add(listener) }
     ...
     * @return This shell instance for chaining calls.
     */
    fun removeOnStdoutLineListener(listener: OnLineListener) =
      apply {
     ...
     * @return This shell instance for chaining calls.
     */
    fun addOnStderrLineListener(listener: OnLineListener) = apply { onStdErrListeners.add(listener) }
     ...
     * @return This shell instance for chaining calls.
     */
    fun removeOnStderrLineListener(listener: OnLineListener) =
      apply {
    ...
    fun run(
      ...
      val onComplete = { marker: Command.Marker ->
        when (marker.uuid) {
          uuid -> {
            try {
              // Reached the end of reading the stream for the command.
      ...
      stderrReader.onReadLine =
        when (config.redirectStdErr) {
      ...
      if (config.notify) {
        onResultListeners.forEach { listener -> listener.onResult(result) }
      }
    ...
    private fun write(vararg commands: String) =
      try {
    ...
    object Command {
      ...
      data class Result(
         ...
         * @property startTime The time—in milliseconds since January 1, 1970, 00:00:00 GMT—when the
         *   command started execution.
         * @property endTime The time—in milliseconds since January 1, 1970, 00:00:00 GMT—when the
    ...
    private class Watchdog : CountDownLatch(STREAM_READER_COUNT) {
       ...
       * @throws AbortedException if the timeout completes before [signal] is called twice or if the
      ...
      fun await(timeout: Timeout?): Boolean =
        when (timeout) {
          null -> {
            ...
            await(timeout.value, timeout.unit)
          }
        }

      override fun await() = super.await().also { if (aborted) throw AbortedException() }

      override fun await(
        timeout: Long,
        unit: TimeUnit,
      ) = super.await(timeout, unit).also { if (aborted) throw AbortedException() }
    ...
    companion object {
      ...
      val instances by lazy { mutableMapOf<String, Shell>() }

      /** Returns a [Shell] instance using the [path] as the path to the shell/executable.\ */
      operator fun get(path: String): Shell =
        instances[path]?.takeIf { shell -> shell.isAlive() }
          ?: Shell(path).also { shell -> instances[path] = shell }
       ...
       * @param environment Map of all environment variables to include with the system environment.
      ...
      ): Process = Runtime.getRuntime().exec(command, (System.getenv() + environment).toArray())
      ...
      private fun Array<out String>.toEnvironmentMap(): EnvironmentMap =
        mutableMapOf<Variable, Value>()
          .also { map ->
            forEach { str ->
              str
                .split("=")
                .takeIf { arr -> arr.size == 2 }
                ?.let { (variable, value) -> map[variable] = value }
            }
          }.toMap()
      ...
      private fun Array<out Pair<Variable, Value>>.toEnvironmentMap(): EnvironmentMap =
        mutableMapOf<Variable, Value>().also { map ->
          forEach { (variable, value) -> map[variable] = value }
        }

      /**
       * Converts an [EnvironmentMap] to an array of strings with the variable/value separated by an
       * '=' character.
       *
      ...
      private fun EnvironmentMap.toArray(): Array<String> =
        mutableListOf<String>()
          .also { list -> forEach { (variable, value) -> list.add("$variable=$value") } }
          .toTypedArray()
    }
  }

@Throws(Shell.NotFoundException::class)
fun Shell.Companion.registerShell(
  path: String,
  environment: EnvironmentMap = emptyMap(),
): Shell = Shell(path, environment).also { shell -> instances[path] = shell }
