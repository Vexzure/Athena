...
object ShizukuUtils {
  ...
  fun requestShizukuPermission(onPermissionGranted: (() -> Unit)? = null) {
    try {
      if (isShizukuAvailable() && !isShizukuPermissionGranted()) {
        ...
        Logger.debug(
          "Shizuku permission request not needed. Available: ${isShizukuAvailable()
          }, Granted: ${isShizukuPermissionGranted()
          }",
        )
  ...
  fun isShizukuReady(): Boolean = isShizukuAvailable() && isShizukuPermissionGranted()
}
