...
class HostsManager(
  ...
  fun isHostsWritable(): Pair<Boolean, Boolean> {
    ...
    val isMagiskSystemless =
      mounts.isNotEmpty() && mounts.contains("magisk") && mounts.contains("bind")

    Logger.info(
      "Hosts can be made writable: $isWritable, Magisk systemless detected: $isMagiskSystemless",
    )
  ...
  private fun createTempHostsFile(): String {
    ...
    tempFile.bufferedWriter().use { writer ->
      ...
      writer.write(
        "# Please do not modify it directly, it will be overwritten when " +
          "Athena is applied again.\n\n",
      )
    ...
    Logger.info(
      "HostsManager: Temp hosts file created at ${tempFile.absolutePath} " +
        "with ${domains.size} blocked domains",
    )
