    ...
    PermissionModal(
      ...
      onPermissionRequest = {
        scope.launch(Dispatchers.Main) {
          ...
          if (result) {
            onEnd()
            onPermissionResult(RootResult.GRANTED)
          } else {
            onEnd()
            onPermissionResult(RootResult.DENIED)
          }
        }
      },
      onPermissionAlternativeRequest = {
        onEnd()
        onPermissionResult(RootResult.VPN)
      },
      permissionAlternative = stringResource(id = R.string.permissions_deny_root),
      onDismiss = {
        onPermissionResult(RootResult.CLOSED)
        onEnd()
      },
    )
  }

  LaunchedEffect(true) { viewModel.updateRootPermissionStatus(true) }
}
