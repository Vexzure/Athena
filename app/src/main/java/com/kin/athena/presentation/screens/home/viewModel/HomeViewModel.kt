...
class HomeViewModel
  ...
  constructor(
    ...
    private val _applicationState =
      mutableStateOf<ApplicationListState>(ApplicationListState.Loading)
    ...
    val firewallClicked: State<Boolean>
      get() = _firewallClicked

    private val _rootUncleaned = mutableStateOf(false)
    val rootUncleaned: State<Boolean>
      get() = _rootUncleaned

    private val _magiskError = mutableStateOf(false)
    val magiskError: State<Boolean>
      get() = _magiskError
    ...
    fun refreshVisibleApplications() {
      ...
      if (currentTime - lastRefreshTime < refreshThrottleMs) {
        Logger.info(
          "HomeViewModel: Refresh throttled - last refresh was " +
      ...
      if (currentState is ApplicationListState.Success) {
        ...
        viewModelScope.launch(Dispatchers.IO) {
          ...
          val freshApps =
            packageIds.mapNotNull { packageId ->
              applicationUseCases.getApplication
                .execute(packageId)
                .fold(ifSuccess = { it }, ifFailure = { null })
            }

          // Update state with fresh data
          val updatedApplications =
            currentState.applications.map { oldApp ->
    ...
    fun updateSearchQuery(query: String) {
      ...
      searchJob =
        viewModelScope.launch {
          ...
          val showSystemPackages =
            currentSettingsViewModel?.settings?.value?.showSystemPackages ?: true
          val showOfflinePackages =
            currentSettingsViewModel?.settings?.value?.showOfflinePackages ?: true

          Logger.info(
            "HomeViewModel: Search with query='$query', " +
              "showSystemPackages=$showSystemPackages, showOfflinePackages=$showOfflinePackages",
          )

          val result =
            applicationUseCases.getFilteredApplications.execute(
          ...
          when (result) {
            ...
            is Result.Failure -> {
              _applicationState.value =
                ApplicationListState.Error(result.error.message ?: "Failed to search applications")
            }
          }
        }
    }
    ...
    fun updateFirewallStatus(
      ...
      serviceType: FirewallMode,
    ) {
      firewallManager.setFirewallMode(serviceType)
      value?.let {
        if (it.name() == FirewallStatus.ONLINE.name()) {
          firewallManager.startFirewall()
        } else if (it.name() == FirewallStatus.OFFLINE.name()) {
    ...
    fun loadApplications(
      ...
      viewModelScope.launch(Dispatchers.IO) {
        try {
          ...
          Logger.info(
            "HomeViewModel: Loading applications with search='${_searchQuery.value}', " +
              "showSystemPackages=$showSystemPackages, showOfflinePackages=$showOfflinePackages",
          )
          ...
          when (result) {
            is Result.Success -> {
              val data = result.data
              Logger.info(
                "HomeViewModel: Loaded ${data.applications.size} apps, totalCount=${data.totalCount}",
              )
              _applicationState.value =
                ApplicationListState.Success(
                  applications = data.applications,
    ...
    fun updatePackage(
      ...
      viewModelScope.launch {
        Logger.info(
          "HomeViewModel: updatePackage called for ${packageEntity.packageID}, " +
            "wifi=${packageEntity.internetAccess}, " +
            "cellular=${packageEntity.cellularAccess}",
        )
        ...
        if (updateUI) {
          ...
          if (currentState is ApplicationListState.Success) {
            val updatedApplications =
              currentState.applications.map { app ->
                if (app.packageID == packageEntity.packageID) packageEntity else app
              }
            _applicationState.value = currentState.copy(applications = updatedApplications)
            Logger.info(
              "HomeViewModel: UI state updated immediately for ${packageEntity.packageID}",
            )
          }
        }

        // Update database on IO thread and wait for completion
        withContext(Dispatchers.IO) { applicationUseCases.updateApplication.execute(packageEntity) }
        ...
        if (firewallManager.rulesLoaded.value == FirewallStatus.ONLINE) {
          withContext(Dispatchers.IO) { firewallManager.updateFirewallRules(packageEntity) }
        }
      }
    }
    ...
    fun togglePinned(packageEntity: Application) {
      viewModelScope.launch {
        ...
        Logger.info(
          "HomeViewModel: togglePinned called for ${packageEntity.packageID}, " +
        ...
        if (currentState is ApplicationListState.Success) {
          ...
          val sortedApplications =
            updatedApplications.sortedWith(
    ...
    fun initialize(settingsViewModel: SettingsViewModel) {
      ...
      settingsViewModel.onAppFilteringSettingsChanged = {
        Logger.info(
          "HomeViewModel: onAppFilteringSettingsChanged triggered, " +
            "reloading packages and applications",
        )
        viewModelScope.launch(Dispatchers.IO) {
      ...
      iconLoadingJob =
        viewModelScope.launch(Dispatchers.IO) {
          ...
          if (priorityApps.isNotEmpty()) {
            ...
            Logger.info(
              "HomeViewModel: Loaded ${priorityIcons.size} priority icons " +
                "(total apps: ${applications.size})",
            )
          }
    ...
    private fun createAppChangeCallback(settingsViewModel: SettingsViewModel) =
      object : AppChangeCallback {
            ...
            viewModelScope.launch {
              try {
                ...
                application.fold(
                  ...
                  ifFailure = { error ->
                    Logger.error(
                      "Failed to get application $it for uninstall cleanup: ${error.message}",
                    )
                  },
                )
              } catch (@Suppress("TooGenericExceptionCaught") e: Exception) {
    ...
    fun deleteApplication(application: Application) {
      viewModelScope.launch { applicationUseCases.deleteApplication.execute(application) }
    }
