...
fun PrivacyScreen(
                ...
                .exportBackup(
                  ...
                  withContext(Dispatchers.Main) {
                    Toast
                      .makeText(
                        context,
                        context.getString(R.string.backup_export_failed, error.message),
                        Toast.LENGTH_LONG,
                      ).show()
                  }
                }
            ...
            context.contentResolver.openInputStream(it)?.use { inputStream ->
              backupManager
                .importBackup(
                  ...
                  onSettingsRestored = { restoredSettings -> settings.update(restoredSettings) },
                ).onSuccess {
                  withContext(Dispatchers.Main) {
                    Toast
                      .makeText(
                  ...
                  withContext(Dispatchers.Main) {
                    Toast
                      .makeText(
                        context,
                        context.getString(R.string.backup_import_failed, error.message),
                        Toast.LENGTH_LONG,
                      ).show()
                  }
                }
  ...
  SettingsScaffold(
    ...
    settingsContainer {
      ...
      SettingsBox(
        isEnabled =
          settings.settings.value.passcode != null ||
            settings.settings.value.pattern != null ||
            settings.settings.value.fingerprint,
  ...
  CustomListDialog(text = stringResource(id = R.string.privacy_app_lock), onExit = onExit) {
    item {
      CreateSettingBox(
        ...
        onAction = {
          ...
          if (settings.settings.value.passcode
            ...
            settings.update(
              settings.settings.value.copy(passcode = null, defaultRoute = HomeRoutes.Home.route),
            )
          ...
          onAction = {
            ...
            if (!settings.settings.value.fingerprint) {
              navController.safeNavigate(
                SettingRoutes.LockScreen.createRoute(ActionType.FINGERPRINT),
              )
            } else {
              settings.update(
      ...
      CreateSettingBox(
        ...
        isEnabled =
          settings.settings.value.pattern
            .isNullOrBlank(),
        onAction = {
          onExit()
          if (settings.settings.value.pattern
              .isNullOrBlank()
          ) {
            navController.safeNavigate(SettingRoutes.LockScreen.createRoute(ActionType.PATTERN))
          } else {
            settings.update(
              settings.settings.value.copy(pattern = null, defaultRoute = HomeRoutes.Home.route),
            )
            settings.loadDefaultRoute()
          }
        },
...
private fun CreateSettingBox(
  ...
  SettingsBox(
    ...
    actionType = SettingType.CUSTOM,
    customAction = { LaunchedEffect(Unit) { onAction() } },
    customButton = {
      Icon(
        imageVector =
          if (isEnabled) {
