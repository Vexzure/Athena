...
fun AboutScreen(
  ...
  SettingsScaffold(
    ...
    item { LogoWithBlob { navController.safeNavigate(HomeRoutes.Debug.route) } }
    item {
      SettingsVerification(
        ...
        description =
          stringResource(id = R.string.about_maintained_by) + " " + ProjectConstants.DEVELOPER,
      )
    }
    settingsContainer {
      SettingsBox(
    ...
    settingsContainer {
      SettingsBox(
        ...
        customAction = { onExit -> PremiumCodeDialog(settings = settings, onExit = { onExit() }) },
      )
    }
  }
}
...
fun PremiumCodeDialog(
  ...
  SettingDialog(text = stringResource(id = R.string.premium_code), onExit = onExit) {
    Column {
      // Premium code input
      Text(
        text = "Premium Code",
