      ...
      content = {
        when (currentStep) {
          ...
          PermissionStep.VPN_PERMISSION -> {
            VpnPermissionRequest(context = context, vpnLauncher = vpnLauncher)
          }
...
private fun VpnMethodSelection(
  ...
  Column(
    ...
    Button(onClick = onVpnSelected, modifier = Modifier.fillMaxWidth()) {
      Text(text = stringResource(R.string.method_use_vpn), fontWeight = FontWeight.Bold)
    }
...
private fun ShizukuPermissionRequest(onPermissionGranted: () -> Unit) {
  LaunchedEffect(Unit) { ShizukuUtils.requestShizukuPermission(onPermissionGranted) }
