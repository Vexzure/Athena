  ...
  constructor(
    ...
    override suspend fun doWork(): Result =
      withContext(Dispatchers.IO) {
        ...
        val hostsToProcess =
          if (targetLists != null && targetLists.isNotEmpty()) {
        ...
        try {
          ...
          Logger.warn(
            "DNS update timeout: Some downloads did not complete within ${DATABASE_UPDATE_TIMEOUT}ms",
          )
        }
        ...
        Logger.info(
          "DNS blocklist $updateType completed: $successCount succeeded, " +
            "$errorCount failed (${duration}ms total)",
        )
        ...
        if (errors.isNotEmpty() && successCount == 0) {
          ...
          if (errors.isNotEmpty()) {
            Logger.info(
              "Completed with $successCount successes and $errorCount errors (partial success)",
            )
          }
          Result.success()
        }
