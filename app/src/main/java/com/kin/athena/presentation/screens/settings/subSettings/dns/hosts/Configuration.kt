...
data class Configuration(
  ...
  fun addURL(
    ...
    location: String,
    state: HostState,
  ) = hosts.items.add(element = HostFile(title = title, data = location, state = state))

  fun removeURL(oldURL: String) = hosts.items.removeAll { it.data == oldURL }
...
data class HostFile(
  ...
  fun isDownloadable(): Boolean = data.startsWith("https://") || data.startsWith("http://")

  companion object : Parceler<HostFile> {
    override fun HostFile.write(
      parcel: Parcel,
      flags: Int,
    ) {
      parcel.apply {
...
data class HostException(
  ...
  companion object : Parceler<HostException> {
    override fun HostException.write(
      parcel: Parcel,
      flags: Int,
    ) {
      parcel.apply {
