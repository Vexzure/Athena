...
fun AppNavHost(
  ...
  NavHost(navController = navController, startDestination = startDestination) {
    ...
    animatedComposable(LogRoutes.Logs.route) {
      LogsScreen(onBackNavClicked = { navController.navigateUp() }, navController = navController)
    }
    ...
    animatedComposable(SettingRoutes.LockScreen.route) { backStackEntry ->
      ...
      LockScreen(settingsViewModel = settings, navController = navController, action = action)
    }

    slideInComposable(SettingRoutes.Settings.route) {
      SettingsScreen(navController = navController, settings = settings)
    }

    animatedComposable(HomeRoutes.Debug.route) { DebugScreen(navController, settings) }

    animatedComposable(LogRoutes.Ips.route) { IpScreen(navController) }
    ...
    settingScreens.forEach { (route, screen) ->
      if (route == SettingRoutes.Settings.route) {
        ...
        animatedComposable(route) { screen(settings, navController, homeViewModel) }
      }
    }
  }
}
