...
class DetailsViewModel
  ...
  constructor(
    ...
    private val _topDomainsAndIps: MutableState<List<Triple<String, Int, String>>> =
          ...
          .fold(
            ifSuccess = { logs ->
              logs?.collectLatest { logs ->
                ...
                val domainAndIpCounts =
                  appLogs
                    .groupingBy { it.destinationAddress ?: it.destinationIP ?: "Unknown" }
                    .eachCount()

                _topDomainsAndIps.value =
                  domainAndIpCounts.entries
                    .sortedByDescending { it.value }
                    .take(5)
                    .map { (key, value) ->
                      val logId =
                        appLogs
                          .firstOrNull { it.destinationAddress == key || it.destinationIP == key }
                          ?.id
                          .toString()
    ...
    private fun loadPackage() {
      applicationID?.let {
        viewModelScope.launch {
          ...
          applicationUseCases.observeApplication
            .execute(applicationID)
            .collectLatest { application ->
              application?.let {
    ...
    fun updatePackage(packageEntity: Application) {
      viewModelScope.launch {
        Logger.info(
          "DetailsViewModel: Updating package ${packageEntity.packageID}, " +
            "wifi=${packageEntity.internetAccess}, cellular=${packageEntity.cellularAccess}, " +
            "bypassVpn=${packageEntity.bypassVpn}",
        )
    ...
    fun isVpnMode(): Boolean = firewallManager.rulesLoaded.value == FirewallStatus.ONLINE
  }
