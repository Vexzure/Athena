...
fun NetworkScreen(
  ...
  val networkViewModel =
    hiltViewModel<IpDialogViewModel>().apply {
    ...
    settingsContainer {
      if (!isShizukuMode) {
        SettingsBox(
          ...
          customAction = { navController.safeNavigate(LogRoutes.Ips.route) },
        )
        SettingsBox(
      ...
      val speedMonitorDescription =
        stringResource(id = R.string.notification_network_speed_channel_desc)
    ...
    if (!isShizukuMode) {
      settingsContainer {
          ...
          customAction = { onExit ->
            LaunchedEffect(Unit) {
              networkViewModel.updateIpv4DialogText(
                TextFieldValue(settings.settings.value.ipv4 ?: ""),
              )
            }
            IpDialog(
              onExit = {
              ...
              onFinished = {
                settings.update(
                  settings.settings.value.copy(
                    ipv4 = networkViewModel.dialogIpv4TextField.value.text,
                  ),
                )
                onExit()
              },
        ...
        SettingsBox(
          ...
          customAction = { onExit ->
            LaunchedEffect(Unit) {
              networkViewModel.updateIpv6DialogText(
                TextFieldValue(settings.settings.value.ipv6 ?: ""),
              )
            }
            ...
            IpDialog(
              ...
              onFinished = {
                settings.update(
                  settings.settings.value.copy(
                    ipv6 = networkViewModel.dialogIpv6TextField.value.text,
                  ),
                )
                onExit()
              },
