  ...
  SettingsScaffold(
    ...
    settingsContainer {
      ...
      SettingsBox(
        ...
        onNotUsableClicked = { behaviorViewModel.showFirewallDisableMessage() },
          ...
          if (isFirewallOnline) {
            ...
            if (settings.settings.value.useRootMode == null) {
              CoroutineScope(Dispatchers.IO).launch {
                if (grantRootAccess()) {
                  settings.update(
                    settings.settings.value.copy(useRootMode = true, useShizukuMode = null),
                  )
                }
              }
            } else {
          ...
          if (isFirewallOnline) {
            ...
            if (settings.settings.value.useShizukuMode == null) {
              if (ShizukuUtils.isShizukuReady()) {
                settings.update(
                  settings.settings.value.copy(useShizukuMode = true, useRootMode = null),
                )
              } else {
      ...
      SettingsBox(
        ...
        onNotUsableClicked = { behaviorViewModel.showFirewallDisableMessage() },
        variable =
          settings.settings.value.useRootMode == null &&
            settings.settings.value.useShizukuMode == null,
        onSwitchEnabled = {
          if (isFirewallOnline) {
            behaviorViewModel.showFirewallDisableMessage()
          } else {
            if (it) {
              settings.update(
                settings.settings.value.copy(useRootMode = null, useShizukuMode = null),
              )
            }
          }
        },
    ...
    settingsContainer {
      SettingsBox(
        ...
        onSwitchEnabled = { settings.update(settings.settings.value.copy(startOnBoot = it)) },
      )
    }
    settingsContainer {
      SettingsBox(
        ...
        onSwitchEnabled = {
          settings.update(
            settings.settings.value.copy(showSystemPackages = it),
          )
        },
      )
      SettingsBox(
