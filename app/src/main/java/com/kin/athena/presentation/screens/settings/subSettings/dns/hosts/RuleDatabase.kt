...
class RuleDatabase {
  ...
  suspend fun initialize(
    ...
    progressCallback: (suspend (Int) -> Unit)? = null,
  ): List<String>? {
    val sortedHostItems =
      config.hosts.items
        .mapNotNull { if (it.state != HostState.IGNORE) it else null }
    ...
    Logger.info(
      "Domain rules loaded: $totalRules total " +
  ...
  private fun loadReader(
    ...
    try {
      ...
      Logger.info(
        "${item.title}: Loaded $count domains, $wildcardCount wildcards, " +
          "$regexCount regex, $whitelistCount whitelisted",
      )
      return hosts
    } catch (e: IOException) {
