...
class SettingsViewModel
  ...
  constructor(
            ...
            ifSuccess = { licenseResponse ->
              if (licenseResponse.valid == true) {
                ...
                Toast
                  .makeText(
                    context,
                    context.getString(R.string.premium_activation_success),
                    Toast.LENGTH_LONG,
                  ).show()
                onResult(true, "✅ ${context.getString(R.string.premium_activation_success)}")
              } else {
                val errorMessage =
                  licenseResponse.error ?: context.getString(R.string.premium_invalid_license)
                Toast
                  .makeText(
                    context,
                    context.getString(R.string.premium_activation_error, errorMessage),
                    Toast.LENGTH_LONG,
                  ).show()
    ...
    fun getProductPrice(productId: String): String? = billingProvider.getBillingInterface()?.getProductPrice(productId)

    fun getDiscountText(): String = billingProvider.getBillingInterface()?.getDiscountText() ?: "20% OFF"
    ...
    fun showFeatureChoiceDialog(
      ...
      println(
        "DEBUG: SettingsViewModel(${this.hashCode()}) - Dialog state is now: " +
          "${_showFeatureChoiceDialog.value}",
      )
    }
    ...
    fun updateDefaultRoute(route: String) {
      ...
      viewModelScope.launch { preferenceUseCases.saveSettings.execute(_settings.value) }
    }
    ...
    fun update(
      ...
      if (
        oldSettings.showSystemPackages != newSettings.showSystemPackages ||
        oldSettings.showOfflinePackages != newSettings.showOfflinePackages
      ) {
        Logger.info(
          "SettingsViewModel: App filtering settings changed, triggering immediate app reload",
        )
      ...
      viewModelScope.launch {
        try {
          preferenceUseCases.saveSettings
            .execute(newSettings)
            .fold(
              ifSuccess = {
    ...
    fun getSupportedLanguages(context: Context): Map<String, String> {
      ...
      for (a in 0 until localeList.size()) {
        localeList[a].let { it?.let { it1 -> map.put(it1.getDisplayName(it), it.toLanguageTag()) } }
      }
      return map
    }

    // Taken from:
    // https://stackoverflow.com/questions/74114067/get-list-of-locales-from-locale-config-in-android-13
