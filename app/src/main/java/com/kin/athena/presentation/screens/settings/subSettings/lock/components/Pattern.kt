...
fun PatternLock(
  ...
  Column(
    ...
    val text =
      when {
        patternStatus.isNotEmpty() -> {
          patternStatus
        }

        settingsViewModel.settings.value.pattern == null -> {
          stringResource(id = R.string.lock_setup_pattern)
        }

        settingsViewModel.settings.value.pattern != null -> {
          stringResource(id = R.string.lock_pattern)
        }
    ...
    Canvas(
                ...
                lockScreenViewModel.canvasSize
                  ...
                  ?.let { size ->
                    ...
                    if (cell != null) {
                      ...
                      lockScreenViewModel.path =
                        Path().apply { moveTo(cell.center.x, cell.center.y) }
                ...
                if (lockScreenViewModel.selectedCellsIndexList.isNotEmpty()) {
                  ...
                  lockScreenViewModel.canvasSize
                    ...
                    ?.let { size ->
                      ...
                      if (
                        cell != null && cell.index !in lockScreenViewModel.selectedCellsIndexList
                      ) {
                        lockScreenViewModel.selectedCellsIndexList.add(cell.index)
                        lockScreenViewModel.selectedCellCenterList.add(cell.center)
              ...
              MotionEvent.ACTION_UP -> {
                ...
                if (lockScreenViewModel.selectedCellsIndexList.isNotEmpty()) {
                  if (settingsViewModel.settings.value.pattern == null) {
                    ...
                    if (
                      settingsViewModel.settings.value.pattern ==
                      lockScreenViewModel.selectedCellsIndexList.joinToString("")
                    ) {
      ...
      for (row in 0 until rowCount) {
        for (column in 0 until columnCount) {
          val center =
            Offset((boxCenterInX + boxSizeInX * column), (boxCenterInY + boxSizeInY * row))
          ...
          val currentDotColor =
            when {
              patternStatus == "Wrong Pattern" -> errorColor

              // All dots red on error
      ...
      lockScreenViewModel.currentTouchOffset?.let { offset ->
        lockScreenViewModel.lastCellCenter?.let { lastCenter ->
          ...
          drawPath(
            ...
            brush =
              Brush.linearGradient(
                listOf(pathColor.copy(alpha = 0.7f), pathColor.copy(alpha = 0.3f)),
          ...
          drawCircle(color = pathColor.copy(alpha = 0.8f), radius = 8f, center = offset)
        }
      }
    }
  }
