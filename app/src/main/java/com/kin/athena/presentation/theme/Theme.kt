...
fun EasyWallTheme(
  ...
  if (settingsModel.settings.value.automaticTheme) {
    settingsModel.update(
      settingsModel.settings.value.copy(
  ...
  val useSystemDynamic =
    settingsModel.settings.value.customColor ==
      -7896468 // Use system colors if user hasn't picked custom color
  MaterialTheme(
    colorScheme =
      getColorScheme(
        context,
