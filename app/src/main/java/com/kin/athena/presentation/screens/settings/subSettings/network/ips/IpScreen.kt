  ...
  MaterialScaffold(
    ...
    Column(modifier = Modifier.fillMaxSize().padding(horizontal = 16.dp)) {
      if (ipViewModel.ips.value.isEmpty()) {
        ...
        val filteredIps =
          remember(ipViewModel.ips.value, ipViewModel.query.value) {
        ...
        LazyColumn(
          ...
          settingsContainer {
            filteredIps.forEach { ip ->
              IpCard(ip = ip.ip, onDelete = { ipViewModel.deleteIP(ip.ip) })
            }
          }
        }
      }
...
private fun IpCard(
  ...
  if (showDeleteConfirm) {
    SettingDialog(text = "Delete IP Address", onExit = { showDeleteConfirm = false }) {
      Column {
        Text(
