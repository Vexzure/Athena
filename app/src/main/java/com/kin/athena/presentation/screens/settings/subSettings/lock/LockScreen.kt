...
fun LockScreen(
  ...
  MaterialScaffold {
    if (action != null) {
      when (action) {
        ActionType.PASSCODE -> {
          PasscodeLock(settingsViewModel, navController)
        }

        ActionType.FINGERPRINT -> {
          FingerprintLock(settingsViewModel = settingsViewModel, navController = navController)
        }

        ActionType.PATTERN -> {
          PatternLock(settingsViewModel = settingsViewModel, navController = navController)
        }
      }
    } else {
      when {
        ...
        settingsViewModel.settings.value.fingerprint -> {
          FingerprintLock(settingsViewModel = settingsViewModel, navController = navController)
        }

        settingsViewModel.settings.value.passcode != null -> {
          PasscodeLock(settingsViewModel, navController)
        }
      }
    }
