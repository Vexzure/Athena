...
object BlocklistParser {
  ...
  fun parseLine(line: String): BlocklistRule? {
    ...
    return when (firstChar) {
      ...
      'a' -> {
        parseDNSmasqFormat(trimmedLine) ?: parseHostsFormat(trimmedLine)
      }
  ...
  /**
   * Parse ABP (Adblock Plus) format Examples:
  ...
  private fun parseABPFormat(line: String): BlocklistRule? {
    ...
    if (line.startsWith("@@||") && line.contains("^")) {
      val domain = line.substring(4, line.indexOf("^")).lowercase().trim()
    ...
    if (line.startsWith("||") && line.contains("^")) {
      val domain = line.substring(2, line.indexOf("^")).lowercase().trim()
  ...
  /**
   * Parse Pi-hole style regex format (raw regex without delimiters) Examples:
   * - ^ad[s]?[0-9]*\.
   * - (^|\.)ad[s]?\d*\. This must come after other formats to avoid false positives
   */
  ...
  /**
   * Parse wildcard format Examples:
   * - *.doubleclick.net
   * - ads.*.com
   * - *.ads.*
   */
  ...
  /**
   * Parse traditional hosts file format Examples:
  ...
  /** Convert wildcard pattern to regex Example: *.example.com -> ^.*\.example\.com$ */
