...
@Singleton
class CustomDomainRepositoryImpl
  @Inject
  constructor(
    private val customDomainDao: CustomDomainDao,
  ) : CustomDomainRepository {
    override fun getAllowlistDomains(): Flow<List<CustomDomain>> =
      customDomainDao.getDomainsByType(isAllowlist = true).map {
        it.toDomainList()
      }

    override fun getBlocklistDomains(): Flow<List<CustomDomain>> =
      customDomainDao.getDomainsByType(isAllowlist = false).map {
        it.toDomainList()
      }

    override fun getEnabledAllowlistDomains(): Flow<List<CustomDomain>> =
      customDomainDao.getEnabledDomainsByType(isAllowlist = true).map {
        it.toDomainList()
      }

    override fun getEnabledBlocklistDomains(): Flow<List<CustomDomain>> =
      customDomainDao.getEnabledDomainsByType(isAllowlist = false).map {
        it.toDomainList()
      }

    override fun getAllDomains(): Flow<List<CustomDomain>> =
      customDomainDao.getAllDomains().map {
        it.toDomainList()
      }

    override suspend fun getDomainById(id: Long): CustomDomain? = customDomainDao.getDomainById(id)?.toDomain()

    override suspend fun isDomainExists(
      domain: String,
      isAllowlist: Boolean,
    ): Boolean = customDomainDao.isDomainExists(domain, isAllowlist)

    override suspend fun insertDomain(domain: CustomDomain): Long = customDomainDao.insertDomain(domain.toEntity())
    ...
    override suspend fun updateDomainEnabled(
      id: Long,
      isEnabled: Boolean,
    ) {
    ...
    override suspend fun getAllowlistCount(): Int = customDomainDao.getDomainCountByType(isAllowlist = true)
