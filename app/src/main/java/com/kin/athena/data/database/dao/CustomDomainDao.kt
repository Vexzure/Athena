...
interface CustomDomainDao {
  @Query(
    "SELECT * FROM custom_domains WHERE is_allowlist = :isAllowlist " +
  ...
  @Query(
    "SELECT EXISTS(SELECT 1 FROM custom_domains WHERE domain = :domain " +
      "AND is_allowlist = :isAllowlist)",
  )
  suspend fun isDomainExists(
    domain: String,
    isAllowlist: Boolean,
  ): Boolean
  ...
  suspend fun updateDomainEnabled(
    id: Long,
    isEnabled: Boolean,
  )
