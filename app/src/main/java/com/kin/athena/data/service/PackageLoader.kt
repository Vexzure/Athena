...
class PackageLoader(
  ...
  suspend fun loadPackages(): Result<Unit, Error> =
    withContext(Dispatchers.IO) {
      return@withContext try {
        ...
        Logger.info(
          "PackageLoader: Settings - showSystem=$showSystemPackages, " +
            "showOffline=$showOfflinePackages, wifiDefault=$wifiDefault, " +
            "cellularDefault=$cellularDefault",
        )
        ...
        Logger.info(
          "PackageLoader: Found ${allApps.size} total apps, " +
            "filtered to ${filteredApps.size} apps " +
            "(removed ${allApps.size - filteredApps.size} apps)",
        )
  ...
  private suspend fun updateEmptyDisplayNames() {
    try {
      ...
      allAppsResult.fold(
        ifSuccess = { allApps ->
          val appsToUpdate =
            allApps.filter { it.displayName.isEmpty() || it.displayName == it.packageID }

          if (appsToUpdate.isNotEmpty()) {
            Logger.info(
              "PackageLoader: Updating display names for ${appsToUpdate.size} applications",
            )
