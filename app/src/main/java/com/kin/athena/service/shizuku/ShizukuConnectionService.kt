...
class ShizukuConnectionService :
  ...
  override fun onCreate() {
    ...
    Logger.debug(
      "ShizukuConnectionService: logUseCases initialized: ${::logUseCases.isInitialized}",
    )
    Logger.debug(
      "ShizukuConnectionService: firewallManager initialized: ${::firewallManager.isInitialized}",
    )
  ...
  override fun stopService(context: Context) {
    ...
    launch { disableFirewallChainSafely() }
  ...
  override fun updateRules(application: Application?) {
    application?.let { app -> launch { setAppNetworking(app) } }
  }
  ...
  override fun isDnsBlockingEnabled(): Boolean = false
  ...
  private suspend fun loadApplications() {
    runBlocking {
      applicationUseCases.getApplications
        .execute()
        .fold(
          ifSuccess = { installedApplications = it },
          ifFailure = { err ->
    ...
    installedApplications?.let {
      ...
      if (::logUseCases.isInitialized) {
        ...
        Logger.error(
          "ShizukuConnectionService: Cannot start logging - logUseCases not initialized!",
        )
      }
    }
  }
  ...
  private fun parseNetworkConnections(
    data: String,
    protocol: String,
  ) {
    if (!isLoggingEnabled) return
  ...
  private fun parseConnectionLine(
    line: String,
    protocol: String,
  ) {
    try {
      ...
      if (destIP != "0.0.0.0" && destPort != "0") {
        ...
        Logger.debug(
          "Logging connection: $protocol UID=$uid $destIP:$destPort (allowed=$isAllowed)",
        )
        ...
        launch {
          if (::logUseCases.isInitialized) {
            ...
            result.fold(
              ...
              ifFailure = { error -> Logger.error("Failed to log connection: ${error.message}") },
            )
          } else {
  ...
  private suspend fun setAppNetworking(app: Application) {
    ...
    if (svc != null && isServiceBound) {
      try {
        ...
        if (success) {
          Logger.debug(
            "ShizukuConnectionService: setPackageNetworking for ${app.packageID} = $allow succeeded",
          )
        } else {
          Logger.warn(
