...
class NflogManager
  ...
  constructor(
          ...
          override fun onLine(line: String) {
            when {
              ...
              line.startsWith("Process ID") -> {
                val extractedPid = extractPidFromOutput(line)
                extractedPid?.let {
                  pid = extractedPid
                  CoroutineScope(Dispatchers.IO).launch {
                    preferencesUseCases.saveSettings.execute(
                      settings = settings.copy(PID = extractedPid),
                    )
                  }
                }
              }
    ...
    fun killService(pid: Int) {
      ...
      commands.forEach { commnad -> shell.run(commnad) }
    }
        ...
        try {
          ...
          val log =
            Log(
              ...
              packetStatus =
                if (application?.cellularAccess != false && application?.internetAccess != false) {
                  FirewallResult.ACCEPT
                } else {
                  FirewallResult.DROP
                },
    ...
    fun saveLog(log: Log) {
      CoroutineScope(Dispatchers.IO).launch { logUseCases.addLog.execute(log) }
    }
  }
