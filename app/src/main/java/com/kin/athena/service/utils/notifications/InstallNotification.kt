...
suspend fun Service.showInstallNotification(
  ...
  val channelID =
    CoreNotificationUtils.createNotificationChannel(
      context = this,
        ...
        .fold(
          ifSuccess = { settings ->
            ...
            applicationToUse?.let { app ->
              applicationUseCases.addApplication
                .execute(app)
                .fold(
                  ifSuccess = {
            ...
            } ?: run { Logger.error("Failed to create application for package $packageName") }
          },
        )
    }
  }
...
private fun Service.createApplication(
  packageName: String,
  settings: Settings,
): Application? =
  try {
...
private fun Service.createInstallNotification(
  ...
  val serviceClass =
    if (useRootMode) RootConnectionService::class.java else VpnConnectionServer::class.java
...
private fun Service.createToggleWifiAction(
  ...
  val toggleWifiIntent =
    createPendingIntent(packageName, serviceClass, NetworkConstants.ACTION_TOGGLE_WIFI)
...
private fun Service.createToggleCellularAction(
  ...
  val toggleCellularIntent =
    createPendingIntent(packageName, serviceClass, NetworkConstants.ACTION_TOGGLE_CELLURAL)
  return NotificationCompat.Action
    .Builder(
      android.R.drawable.ic_menu_close_clear_cancel,
...
private fun Service.getEnableDisableText(isEnabled: Boolean): String =
  if (isEnabled) {
    this.getString(R.string.common_disable)
  } else {
    this.getString(R.string.common_enable)
  }
