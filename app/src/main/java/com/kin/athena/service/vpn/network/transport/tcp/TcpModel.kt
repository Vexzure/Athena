...
sealed class TCPOption {
  ...
  data class WindowScale(
    val scaleFactor: UByte,
  ) : TCPOption() {
  ...
  companion object {
    fun parse(buffer: ByteBuffer): TCPOption {
      ...
      return when (kind) {
        TcpConstants.OPT_KIND_EOL -> {
          EndOfOptionList
        }

        TcpConstants.OPT_KIND_NOP -> {
          NoOperation
        }
          ...
          if (!buffer.hasRemaining()) {
            throw IllegalArgumentException(
              "Buffer underflow: cannot read option length for kind $kind.",
            )
          }
          ...
          if (length < 2u) {
            throw IllegalArgumentException(
              "Invalid option length $length for kind $kind (must be >= 2).",
            )
          }
          ...
          if (buffer.remaining() < dataBytesLength) {
            throw IllegalArgumentException(
              "Buffer underflow: option kind $kind, length $length, " +
                "needs $dataBytesLength data bytes, found ${buffer.remaining()}.",
            )
          }
...
data class TCPHeader(
  ...
  var reserved: Int = 0
    set(value) {
      if (value < 0 || value > 7) {
        throw IllegalArgumentException("Reserved field must be between 0 and 7 (3 bits).")
      }
      field = value
    }
  ...
  val dataOffset: Int
    get() {
      ...
      for (option in options) {
        optionsLengthBytes +=
          when (option) {
            is TCPOption.EndOfOptionList,
            is TCPOption.NoOperation,
            -> 1
  ...
  fun calculateChecksum(
    ...
    val fullBuffer =
      ByteBuffer.allocate(totalSize + (if (totalSize % 2 != 0) 1 else 0)) // Pad to even bytes
  ...
  companion object {
    ...
    private fun logMalformedPacket(
      reason: String,
      ...
      if (currentTime - lastMalformedLogTime > LOG_INTERVAL_MS) {
        ...
        Logger.warn(
          "TCP malformed packets detected ($malformedPacketCount total): $reason | Packet hex: $packetHex",
        )
    ...
    fun fromByteBuffer(buffer: ByteBuffer): TCPHeader {
      ...
      if (buffer.remaining() < TcpConstants.MIN_HEADER_LENGTH_BYTES) {
        throw IllegalArgumentException(
          "Buffer too small for TCP header. " +
            "Min ${TcpConstants.MIN_HEADER_LENGTH_BYTES} bytes required, got ${buffer.remaining()}.",
        )
      }
      buffer.order(ByteOrder.BIG_ENDIAN)

      val sourcePort =
        buffer.getShort(originalPosition + TcpConstants.OFFSET_SOURCE_PORT).toInt() and 0xFFFF
      val destinationPort =
        buffer.getShort(originalPosition + TcpConstants.OFFSET_DESTINATION_PORT).toInt() and 0xFFFF
      val sequenceNumber =
        buffer.getInt(originalPosition + TcpConstants.OFFSET_SEQUENCE_NUMBER).toLong() and
          0xFFFFFFFFL
      val acknowledgmentNumber =
        buffer.getInt(originalPosition + TcpConstants.OFFSET_ACKNOWLEDGMENT_NUMBER).toLong() and
          0xFFFFFFFFL

      val byte12 =
        buffer.get(originalPosition + TcpConstants.OFFSET_DATA_OFFSET_RESERVED_NS).toInt() and 0xFF
      val parsedDataOffset =
        (byte12 and TcpConstants.DATA_OFFSET_MASK) ushr TcpConstants.DATA_OFFSET_SHIFT
      ...
      val windowSize =
        buffer.getShort(originalPosition + TcpConstants.OFFSET_WINDOW_SIZE).toInt() and 0xFFFF
      val checksum =
        buffer.getShort(originalPosition + TcpConstants.OFFSET_CHECKSUM).toInt() and 0xFFFF
      val urgentPointer =
        buffer.getShort(originalPosition + TcpConstants.OFFSET_URGENT_POINTER).toInt() and 0xFFFF
      ...
      if (actualHeaderLengthBytes < TcpConstants.MIN_HEADER_LENGTH_BYTES) {
        ...
        bufferCopy.position(originalPosition)
        logMalformedPacket(
          "Data Offset $parsedDataOffset: calculated header length " +
            "$actualHeaderLengthBytes bytes is less than minimum " +
            "${TcpConstants.MIN_HEADER_LENGTH_BYTES} bytes",
          bufferCopy,
        )
        throw IllegalArgumentException(
          "Malformed TCP packet: Data Offset $parsedDataOffset produces invalid header length",
        )
      }
      if (buffer.remaining() < actualHeaderLengthBytes - (buffer.position() - originalPosition)) {
        throw IllegalArgumentException(
          "Buffer remaining bytes (${buffer.remaining()}) insufficient for " +
            "declared header length ($actualHeaderLengthBytes bytes) from current position.",
        )
      }
      ...
      ).apply { this.reserved = parsedReserved }
    }

    @Synchronized
    private fun parseOptions(
      optionsBuffer: ByteBuffer,
      optionsList: MutableList<TCPOption>,
    ) {
  ...
  fun toByteArray(): ByteArray {
    ...
    if (calculatedDataOffset < 5 || calculatedDataOffset > 15) {
      throw IllegalStateException(
        "Calculated Data Offset $calculatedDataOffset is out of TCP valid range (5-15).",
      )
    }
    ...
    val byte12 =
      ((calculatedDataOffset and 0xF) shl TcpConstants.DATA_OFFSET_SHIFT) or
    ...
    if (buffer.position() != totalHeaderLengthBytes) {
      throw IllegalStateException(
        "Serialization error: Buffer position ${buffer.position()} " +
          "does not match calculated total header length $totalHeaderLengthBytes. " +
          "Options written: $currentOptionsBytesCount, Padding: ${optionsAreaLength - currentOptionsBytesCount}.",
      )
    }
