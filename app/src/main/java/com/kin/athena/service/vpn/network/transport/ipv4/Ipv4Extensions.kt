...
private fun ByteArray.fillBasicHeaderFields(header: IPv4) {
  ...
  this[6] =
    (
      ((header.fragmentOffset.toInt() shr 8) and 0x1F) or
        if (header.isFragmentationAllowed) 0x40 else 0
    ).toByte()
...
private fun ByteBuffer.checkMinimumHeaderLength() {
  if (remaining() < 20) {
    Logger.error(
      "Minimum IPv4 header length is 20 bytes. There are less than 20 bytes available from the current position.",
    )
  }
}
