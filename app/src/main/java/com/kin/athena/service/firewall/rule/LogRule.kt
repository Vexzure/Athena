      ...
      if (enabled != null) {
        ...
        externalScope.launch(Dispatchers.IO) {
          preferencesUseCases.loadSettings
            .execute()
            .fold(
              ifSuccess = { settings -> mutex.withLock { isLogEnabled = settings.logs } },
              ifFailure = { error -> Logger.error("Failed to load settings: ${error.message}") },
            )
        }
      }
    }
          ...
          if (isLogEnabled && packet.shouldLog) {
            val log =
              Log(
                ...
                destinationAddress =
                  dnsModel?.domainName ?: packet.destinationIP.resolveIpToHostname(),
              )
            try {
