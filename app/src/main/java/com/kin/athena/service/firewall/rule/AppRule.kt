...
class AppRule
  ...
  constructor(
      ...
      CoroutineScope(Dispatchers.IO).launch {
        ...
        packagesList.fold(
          ifSuccess = { applications ->
            ...
            val cellularBlocked =
              applications.filter { !it.cellularAccess }.map { it.uid }.toIntArray()
    ...
    override fun check(
      ...
      val isBlocked =
        when (networkConnectionStateManager.getCurrentConnectionType()) {
          NetworkManager.ConnectionType.WIFI -> {
            packet.uid in wifiBlockedUids
          }
          ...
          NetworkManager.ConnectionType.NONE -> {
            false
          }
        }
