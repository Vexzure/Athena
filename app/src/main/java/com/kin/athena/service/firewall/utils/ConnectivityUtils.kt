...
class ConnectivityUtils
  ...
  constructor(
    ...
    private fun getUidForConnectionOldMethod(fireWallModel: FireWallModel): Int {
      ...
      for (line in connectionList.drop(1)) {
        ...
        if (columns.size >= 8) {
          ...
          if (
            localIp == fireWallModel.sourceIP &&
            localPort == fireWallModel.sourcePort &&
            remoteIp == fireWallModel.destinationIP &&
            remotePort == fireWallModel.destinationPort
          ) {
            return uid
          }
        }
    ...
    private fun parseHexToPort(hexPort: String): Int = hexPort.toInt(16)
  }
