...
class RootConnectionService :
    ...
    intent?.action?.let { action ->
      when (action) {
        NetworkConstants.ACTION_TOGGLE_CELLURAL -> {
          toggleAccess(intent.getStringExtra("packageName"), AccessType.CELLULAR)
        }

        NetworkConstants.ACTION_TOGGLE_WIFI -> {
          toggleAccess(intent.getStringExtra("packageName"), AccessType.WIFI)
        }

        NetworkConstants.ACTION_STOP_ROOT -> {
          onStopIntent()
        }

        NetworkConstants.ACTION_START_ROOT -> {
          onStartIntent()
        }
      }
    }
  ...
  private fun toggleAccess(
    ...
    packageName?.let { pkgName ->
      launch {
        packageManager.getApplication
          .execute(pkgName)
          .fold(
            ifSuccess = { application ->
              val updatedApplication = application.toggleAccess(accessType)
              packageManager.updateApplication
                .execute(updatedApplication)
                .fold(
                  ifSuccess = {
            ...
            ifFailure = { onAppInstalled(packageName) },
          )
      }
    }
  }
  ...
  override fun updateRules(application: Application?) {
    application?.let { app -> launch { updateFirewallRules(app) } }
  }
  ...
  private fun updateFirewallRules(application: Application) {
    ...
    shell.run(
      "iptables -I EasyApps-data -m owner --uid-owner ${application.uid} -j $cellularRule -w",
    )
  ...
  override fun startService(context: Context) {
    context.startService(
      Intent(context, RootConnectionService::class.java).apply {
  ...
  override fun isDnsBlockingEnabled(): Boolean = firewallRulesHandler.isDnsBlockingEnabled()
  ...
  private suspend fun loadRules() {
    ...
    val ipsJob =
      async {
        networkFilterManager.getIps.execute().fold(ifSuccess = { ipAddresses = it.first() })
      }
  ...
  private suspend fun getAllInstalledAppsFromDevice(): List<Application> =
    withContext(Dispatchers.IO) {
      // Get all apps from database
      val dbApps =
        packageManager.getApplications
          .execute()
          .fold(ifSuccess = { it }, ifFailure = { emptyList() })
      ...
      Logger.info(
        "RootConnectionService: Loaded ${mergedApps.size} total apps " +
          "(${dbApps.size} from DB, ${mergedApps.size - dbApps.size} system apps not in DB)",
      )
      mergedApps
    }
  ...
  suspend fun setupEasyAppsChain(
    ...
    val internetApps =
      applications.filter { it.requiresNetworkPermissions(serviceContext.packageManager) }
  ...
  private fun createRejectChain(
    ...
    commands.addAll(
      listOf(
        ...
        "$iptables -A EasyApps-reject -j REJECT --reject-with ${if (iptables == "iptables") {
  ...
  private fun createAppRules(
    iptables: String,
    internetApps: List<Application>,
    commands: MutableList<String>,
  ) {
    internetApps.forEach { application ->
      if (application.internetAccess) {
        commands.add(
          "$iptables -A EasyApps-wifi -m owner --uid-owner ${application.uid} -j ACCEPT -w",
        )
      }
      if (application.cellularAccess) {
        commands.add(
          "$iptables -A EasyApps-data -m owner --uid-owner ${application.uid} -j ACCEPT -w",
        )
      }
    }
  }

  private fun applyIpRules(
    iptables: String,
    ips: List<Ip>,
    commands: MutableList<String>,
  ) {
    ips.forEach { ip -> commands.add("$iptables -I EasyApps-ips -d ${ip.ip} -j DROP -w") }
  }

  private fun closeChains(
    iptables: String,
    commands: MutableList<String>,
  ) {
    commands.addAll(
  ...
  fun isNumber(input: String): Boolean = input.toDoubleOrNull() != null
  ...
  private fun executeCommandsWithProgress(commands: List<String>) {
    ...
    commands.forEachIndexed { index, command ->
      try {
        Logger.info(
          "RootConnectionService: Executing command ${index + 1}/$totalCommands: $command",
        )
  ...
  private fun executeShellCommand(
    ...
    shellExecutor.addOnStdoutLineListener(
  ...
  override fun onAppInstalled(packageName: String?) {
    launch {
      packageName?.let {
        showInstallNotification(it, applicationUseCases, preferencesUseCases, true, firewallManager)
      }
    }
  }
