...
class RuleHandler
  ...
  constructor(
    ...
    init {
      ...
      CoroutineScope(Dispatchers.IO).launch {
        preferencesUseCases.loadSettings
          .execute()
          .fold(ifSuccess = { settings -> allowLocal = settings.allowLocal })
      }
    ...
    suspend fun updateBlocklist(progressCallback: (suspend (Int) -> Unit)? = null) {
      rules.filterIsInstance<DNSRule>().forEach { it.updateBlocklist(progressCallback) }
    }

    fun updateLogs(enabled: Boolean) {
      rules.filterIsInstance<LogRule>().forEach { it.updateLogStatus(enabled) }
    }
    ...
    fun isCustomDomainRulesEnabled(): Boolean =
      rules.filterIsInstance<CustomDomainRule>().firstOrNull()?.isCustomDomainRulesEnabled()
        ?: false

    fun updateScreenSetting() {
      rules.filterIsInstance<ScreenRule>().forEach { it.updateRules() }
    }

    fun updateHttpSettings() {
      rules.filterIsInstance<HTTPRule>().forEach { it.updateHTTPStatus() }
    }

    fun updateAppRules(updatedApplication: Application? = null) {
      rules.filterIsInstance<AppRule>().forEach { it.observePackages(updatedApplication) }
      rules.filterIsInstance<FilterRule>().forEach { it.observeIps() }
    }
    ...
    fun handle(
      ...
      bypassCheck: Boolean,
    ): Triple<Boolean, Int, FirewallResult> {
      if (bypassCheck) {
