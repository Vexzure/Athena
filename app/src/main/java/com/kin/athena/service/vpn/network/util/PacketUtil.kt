...
object PacketUtil {
  ...
  fun read(
    ...
    if (offset < 0 || offset >= buffer.size || length < 1 || length > 4) {
      throw IllegalArgumentException(
        "Invalid offset ($offset) or length ($length) for buffer size ${buffer.size}",
      )
    }
    var value = 0
  ...
  private fun getNetworkInt(
    buffer: ByteArray,
    start: Int,
    length: Int,
  ): Int = read(buffer, start, length)
  ...
  fun calculateUDPHeaderChecksum(
    ...
    val buffer =
      ByteBuffer.allocate(bufferSize).apply {
