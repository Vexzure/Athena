...
class ShizukuService
  ...
  constructor() : FirewallService {
    ...
    private fun checkShizukuAvailability() {
      try {
        ...
        if (isShizukuAvailable) {
          ...
          Logger.debug(
            "Shizuku available: $isShizukuAvailable, Permission granted: $isShizukuPermissionGranted",
          )
        } else {
    ...
    override fun updateRules(application: Application?) {
      if (!isShizukuReady()) {
        Logger.warn(
          "ShizukuService: Creating NetworkFilterService with uid $uid, appId $appId, " +
            "vpnMode $vpnMode",
        )
        return
      }
        ...
        try {
          ...
          val success =
            if (isServiceBound && shizukuFirewallService != null) {
              shizukuFirewallService?.setPackageNetworking(app.packageID, allowNetworkAccess)
                ?: false
            } else {
              // Direct execution fallback
              executeConnectivityCommand(
                "cmd connectivity set-package-networking-enabled $allowNetworkAccess ${app.packageID}",
              )
            }

          if (success) {
            Logger.debug(
              "Successfully updated network rules for ${app.packageID} (UID: ${app.uid}) to $allowNetworkAccess",
            )
          } else {
