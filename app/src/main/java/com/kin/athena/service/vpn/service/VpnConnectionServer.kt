...
class VpnConnectionServer :
  ...
  override fun isDnsBlockingEnabled(): Boolean = ruleManager.isDnsBlockingEnabled()

  override fun updateRules(application: Application?) {
    Logger.info(
      "Updating firewall rules${if (application != null) " for ${application.packageID}" else ""}",
    )
  ...
  override fun onStartCommand(
    ...
    when (intent?.action) {
      NetworkConstants.ACTION_TOGGLE_CELLURAL -> {
        toggleWifiAccess(intent.getStringExtra("packageName"))
      }

      NetworkConstants.ACTION_TOGGLE_WIFI -> {
        toggleCellularAccess(intent.getStringExtra("packageName"))
      }

      NetworkConstants.ACTION_STOP_VPN -> {
        stopVpn()
      }
  ...
  /**
   * Clears all active sessions. This is useful when firewall rules are updated to ensure previously
   * allowed sessions are terminated and new rules are applied.
   */
  fun clearSessions() {
    try {
      if (isShuttingDown) {
        ...
        applicationUseCases.getApplication
          ...
          .fold(
            ifSuccess = { app ->
              ...
              applicationUseCases.updateApplication
                .execute(updatedApp)
                .fold(
                  ifSuccess = {
