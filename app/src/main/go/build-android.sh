...
# Detect OS and set appropriate default Android SDK path
...
# Detect host architecture for toolchain selection
HOST_ARCH=$(uname -m)
if [[ "$OSTYPE" == "darwin"* ]]; then
    if [[ "$HOST_ARCH" == "arm64" ]]; then
        TOOLCHAIN_PATH="$NDK/toolchains/llvm/prebuilt/darwin-arm64/bin"
    else
        TOOLCHAIN_PATH="$NDK/toolchains/llvm/prebuilt/darwin-x86_64/bin"
    fi
else
    if [[ "$HOST_ARCH" == "aarch64" ]]; then
        TOOLCHAIN_PATH="$NDK/toolchains/llvm/prebuilt/linux-arm64/bin"
    else
