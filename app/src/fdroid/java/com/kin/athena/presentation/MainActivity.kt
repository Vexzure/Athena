...
class MainActivity : AppCompatActivity() {
  ...
  @Inject lateinit var billingProvider: BillingProvider
  ...
  private fun handleDeepLink(intent: Intent?) {
    ...
    if (data != null && data.scheme == "athena" && data.host == "premium") {
      val path = data.path
      val licenseKey = data.getQueryParameter("key")

      Logger.info("Deep link received: $data")
      Logger.info("Path: $path, License key: $licenseKey")

      if (path == "/activate" && !licenseKey.isNullOrEmpty()) {
        // Wait for settings to be initialized
        settings?.let { settingsViewModel ->
          settingsViewModel.verifyLicense(licenseKey) { success, message ->
            runOnUiThread {
              Toast
                .makeText(
                  this,
                  message.removePrefix("✅ ").removePrefix("❌ "),
          ...
          ?: run {
            // Settings not ready yet, retry after a short delay
            window.decorView.postDelayed({ handleDeepLink(intent) }, 500)
          }
      }
    }
  }
        ...
        hiltViewModel<SettingsViewModel>().apply {
          ...
          EasyWallTheme(this) {
            ...
            LaunchedEffect(homeViewModel.applicationState.value) {
              if (
                homeViewModel.applicationState.value
                  is com.kin.athena.presentation.screens.home.viewModel.ApplicationListState.Success
              ) {
